        ...
        try {
            $response = $this->openAIClient->chat()->create([
                ...
                'messages' => [
                    ...
                    ['role' => 'system', 'content' => 'For tasks where `is_recurring = false` and `is_fixed = false`, `start_time` and `end_time` represent a **suggested range**, but you may adjust them if needed to improve balance.'],
                    ...
                    ['role' => 'system', 'content' => '1. **First, adjust the task’s time within the available range on the same day while ensuring a comfortable gap between tasks.**'],
                    ['role' => 'system', 'content' => '2. **If necessary, slightly extend or reduce the task’s duration (if flexible) to fit the schedule better.**'],
